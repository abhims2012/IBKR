...
def run_backtest():
    ...
    total_pnl = 0; total_wins = 0; total_losses = 0; trade_size = 500.0
    
    print("--- Original Strategy: TP 5% / SL 5% ---")
    for sym in portfolio:
        try:
            ...
            if df_daily.index.tz is None: df_daily.index = df_daily.index.tz_localize('UTC').tz_convert('America/New_York')
            else: df_daily.index = df_daily.index.tz_convert('America/New_York')
            ...
            in_trade = False; entry_price = 0; wins = 0; losses = 0; pnl = 0
            ...
            for index, row in df.iterrows():
                ...
                if in_trade:
                    if row['High'] >= entry_price * 1.05: # +5% TP
                        ...
                        pnl += profit; wins += 1
                    elif row['Low'] <= entry_price * 0.95: # -5% SL
                        in_trade = False
                        qty = trade_size / entry_price
                        profit = (entry_price * 0.95 - entry_price) * qty
                        pnl += profit; losses += 1
                else:
            ...
            total_wins += wins; total_losses += losses; total_pnl += pnl
        except: pass
    
    total_trades = total_wins + total_losses
    win_rate = (total_wins / total_trades * 100) if total_trades > 0 else 0
    print(f"Total Trades: {total_trades} | Win Rate: {win_rate:.2f}% | PnL: ${total_pnl:.2f}")
