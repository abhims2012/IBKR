...
async def test_stock(ib, symbol):
    ...
    for index, row in df.iterrows():
        if pd.isna(row['SMA_10']) or pd.isna(row['SMA_30']) or pd.isna(row['RSI_14']):
            prev_sma_10 = row['SMA_10']
            prev_sma_30 = row['SMA_30']
            continue
        ...
        if in_trade:
            ...
            # Check for EXACT entry crossover (10 crosses above 30 this week)
            is_crossover = (row['SMA_10'] > row['SMA_30']) and (prev_sma_10 <= prev_sma_30)
            
            # Apply RSI Filter: Only buy if RSI < 70
            if is_crossover and row['RSI_14'] < 70:
                in_trade = True
    ...
    # Force close any open trades at the end to see final PnL
    if in_trade:
...
async def run_backtest():
    ...
    print("--- 5-Year Weekly MA 10/30 + RSI Filter (<70) Backtest ---")
